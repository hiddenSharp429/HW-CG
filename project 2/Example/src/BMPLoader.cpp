...
BMPError BMPLoad(std::string fname,BMPClass& bmp)
{
	...
	char header[54];
	creturn = fread(header,54,1,f);			//read the 54bit main header
	...
	case(24):
		...
		creturn = fread(bmp.bytes,bmp.width*bmp.height*3,1,f);	//24bit is easy
	...
	case(8):
		creturn = fread(cols,256*4,1,f);							//read colortable
		fseek(f,offset,SEEK_SET);
		for(y=0;y<bmp.height;++y)						//(Notice 4bytes/col for some reason)
			for(x=0;x<bmp.width;++x)
			{
				BYTE byte;			
				creturn = fread(&byte,1,1,f);						//just read byte					
				for(int c=0;c<3;++c)
	...
	case(4):
		creturn = fread(cols,16*4,1,f);
		fseek(f,offset,SEEK_SET);
		for(y=0;y<256;++y)
			for(x=0;x<256;x+=2)
			{
				BYTE byte;
				creturn = fread(&byte,1,1,f);						//as above, but need to exract two
	...
	case(1):
		creturn = fread(cols,8,1,f);
		fseek(f,offset,SEEK_SET);
		for(y=0;y<bmp.height;++y)
			for(x=0;x<bmp.width;x+=8)
			{
				BYTE byte;
				creturn = fread(&byte,1,1,f);
