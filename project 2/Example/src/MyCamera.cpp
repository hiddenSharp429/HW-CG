...
namespace tutorial {
    ...
    void MyCamera::draw() {
		...
		glLightfv(GL_LIGHT0, GL_POSITION, lightpos);	// LIGHT MOVES WITH THE CAR?!
        // Camera 
...
//		glTranslated(-_car->_position[0], -_car->_position[1], 0);
		...
		if(_numplayers == 1)
			gluLookAt(_car1->_position[0], _car1->_position[1]-300, 600/1.5, _car1->_position[0],_car1->_position[1],0, 0,1,0);
		else
			gluLookAt((_car1->_position[0]+_car2->_position[0])/2, (_car1->_position[1]+_car2->_position[1])/2-300, 600/1.5, (_car1->_position[0]+_car2->_position[0])/2,(_car1->_position[1]+_car2->_position[1])/2,0, 0,1,0);
